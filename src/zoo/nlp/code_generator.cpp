#include <include/zoo/nlp/code_generator.h>
#include <stdexcept>
#include <fstream>
#include <vector>
#include <string>
...
struct CodeGenerator::Impl {
    CodeGeneratorConfig config_;
    std::unique_ptr<core::InferenceEngine> engine_;
    // std::unique_ptr<preproc::Tokenizer> tokenizer_;
};

CodeGenerator::CodeGenerator(const CodeGeneratorConfig& config)
    : pimpl_(new Impl{config})
{
    if (!std::ifstream(pimpl_->config_.engine_path).good()) {
        throw std::runtime_error("Code generator engine file not found: " + pimpl_->config_.engine_path);
    }
...
std::string CodeGenerator::predict(const std::string& prompt) {
    std::string full_text = prompt;
...
void CodeGenerator::predict_stream(const std::string& prompt,
                                     std::function<void(const std::string&)> stream_callback) {
    if (!pimpl_) throw std::runtime_error("CodeGenerator is in a moved-from state.");
